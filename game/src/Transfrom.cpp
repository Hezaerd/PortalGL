...
void Transfrom::setBulletTransform(MotionState* motion)
{
	btQuaternion rot(
		LowRenderer::degreesToRadians(rotation.Z()),
		LowRenderer::degreesToRadians(rotation.Y()),
		LowRenderer::degreesToRadians(rotation.X()));

	btTransform motionTransform(rot, btVector3(position.X(), position.Y(), position.Z()));
	motion->setWorldTransform(motionTransform);
}
