...
void GameObject::updateGrapMat()
{
	...
	float Rx = LowRenderer::radianToDegrees(atan2(a, b));
	float Ry = LowRenderer::radianToDegrees(atan2(-Matrix[0][2] / Sx, sqrt(a * a + b * b)));
	float Rz = LowRenderer::radianToDegrees(atan2(Matrix[0][1] / Sx, Matrix[0][0] / Sx));

	localTransform.setRotation(lm::vec3(Rx, Ry, Rz));
}

void GameObject::UpdateBulletTranform(MotionState* motion)
{
	localTransform.setBulletTransform(motion);
}
...
void GameObject::UpdateMatrix()
{
	if (!isStatic) {
		if (parent != nullptr) {
			...
			float Rx = LowRenderer::radianToDegrees(atan2(a, b));
			float Ry = LowRenderer::radianToDegrees(atan2(-rotationMatrix[0][2], sqrt(a * a + b * b)));
			float Rz = LowRenderer::radianToDegrees(atan2(rotationMatrix[0][1], rotationMatrix[0][0]));
...
{
	lm::vec3 front;
	front.X() = static_cast<float>(cos(LowRenderer::degreesToRadians(-localTransform.getRotation().Y())) * cos(LowRenderer::degreesToRadians(localTransform.getRotation().Z())));
	front.Y() = static_cast<float>(sin(LowRenderer::degreesToRadians(localTransform.getRotation().Z())));
	front.Z() = static_cast<float>(sin(LowRenderer::degreesToRadians(-localTransform.getRotation().Y())) * cos(LowRenderer::degreesToRadians(localTransform.getRotation().Z())));
